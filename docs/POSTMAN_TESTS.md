...
Os scripts usam Faker (`$randomFullName`, `$randomEmail`, `$randomPhoneNumber`, `$randomStreetAddress`, `$randomInt`) para gerar valores uma vez por cenário. Antes de cada criação válida de carga, a collection consulta os endereços completos de origem e destino em `GET /tracking/geocode`, guarda as coordenadas retornadas e envia no `POST /tracking` apenas os endereços. A resposta da criação deve conter os mesmos endereços e coordenadas consultadas. As datas são calculadas a partir da execução.

As verificações cobrem autorização, validação, duplicidade, paginação, filtros, janelas de datas, isolamento entre clientes, idempotência de localização, entrega, transição de status e atualizações simultâneas. Em `Tracking - Update`, a collection consulta um endereço completo, envia `locationText` exatamente igual e sem latitude/longitude, e a leitura do histórico começa cerca de 2 segundos após o `202`, repetindo a cada segundo por até 15 segundos. O histórico e o detalhe devem preservar o endereço e apresentar as coordenadas obtidas na consulta. As demais leituras de histórico aguardam até 15 segundos para o worker registrar os eventos. `History - Global` também prepara suas duas cargas pela consulta de origem/destino. Os testes que inspecionam Prisma, Redis, BullMQ e o provedor simulado permanecem no E2E; a collection usa o provedor real configurado no ambiente.
...
| Endereço completo de origem | `originAddress` |
| Endereço completo de destino | `destinationAddress` |
| Geocodificação | `geocode`, `GeocodingPort` |
