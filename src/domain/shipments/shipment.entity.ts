...
export function assertCargoCode(value: unknown): string {
  ...
  if (normalized.length > 40) {
    throw new InvalidShipmentError(
      'cargoCode must be at most 40 characters long',
    );
  }
  return normalized;
}
...
export function assertDeliveryWindow(
  ...
  if (estimatedDeliveryDate.getTime() < departureDate.getTime()) {
    throw new ShipmentInvalidDatesError(
      'estimatedDeliveryDate cannot be before departureDate',
    );
  }
  if (estimatedDeliveryDate.getTime() < now.getTime()) {
    throw new ShipmentInvalidDatesError(
      'estimatedDeliveryDate cannot be before the registration date',
    );
  }
}
...
export function validateCreateShipment(
  ...
  const originAddress = assertText(input.originAddress, 'originAddress', 255);
  const destinationCity = assertText(
    input.destinationCity,
    'destinationCity',
    120,
  );
  const destinationCountry = assertText(
    input.destinationCountry,
    'destinationCountry',
    80,
  );
  const destinationAddress = assertText(
    input.destinationAddress,
    'destinationAddress',
    255,
  );
  const departureDate = parseDateInput(input.departureDate, 'departureDate');
  const estimatedDeliveryDate = parseDateInput(
...
export function validateShipmentLocation(
  ...
  const notes =
    ...
    input.notes.trim().length === 0
      ? null
      : assertText(input.notes, 'notes', 500);
