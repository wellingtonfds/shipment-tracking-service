...
export class ShipmentPresenter {
  ...
  @ApiProperty({
    example: 'Avenida Paulista, 1578, São Paulo, SP, Brasil',
    description: 'Complete origin address',
  })
  ...
  @ApiProperty({
    example: 'Avenida Atlântica, 1702, Rio de Janeiro, RJ, Brasil',
    description: 'Complete destination address',
  })
  ...
  @ApiProperty({
    ...
    nullable: true,
    example: 'Campinas, SP',
    description: 'Current location text (location of the latest history event)',
  })
  ...
  @ApiProperty({
    ...
    nullable: true,
    example: -22.9056,
    description: 'Current latitude (location of the latest history event)',
  })
  ...
  @ApiProperty({
    ...
    nullable: true,
    example: -47.0608,
    description: 'Current longitude (location of the latest history event)',
  })
  ...
  @ApiProperty({
    ...
    nullable: true,
    example: null,
    description: 'Delivery date (set when delivered)',
  })
  ...
  @ApiProperty({
    type: Number,
    example: 3,
    description: 'Responsible operator (handler) id',
  })
  handledById!: number;
  ...
  static fromEntity(shipment: ShipmentWithLocation): ShipmentPresenter {
    ...
    presenter.originAddress = shipment.originAddress;
    ...
    presenter.destinationAddress = shipment.destinationAddress;
    presenter.destinationLatitude = shipment.destinationLatitude;
    presenter.destinationLongitude = shipment.destinationLongitude;
    presenter.currentLocationText =
      shipment.currentLocation?.locationText ?? null;
    ...
    presenter.estimatedDeliveryDate =
      shipment.estimatedDeliveryDate.toISOString();
    presenter.deliveredAt = shipment.deliveredAt
...
export class ListShipmentsPresenter {
  @ApiProperty({
    ...
    description: 'Shipments of the page',
  })
  data!: ShipmentPresenter[];

  @ApiProperty({
    type: ListShipmentsMetaPresenter,
    description: 'Pagination metadata',
  })
  meta!: ListShipmentsMetaPresenter;

  static fromPaginated(result: {
    data: ShipmentWithLocation[];
    total: number;
    page: number;
    limit: number;
    totalPages: number;
    ...
    presenter.data = result.data.map((shipment) =>
      ShipmentPresenter.fromEntity(shipment),
    );
